...
<template>
  <div class="layout">
    <aside class="left-nav">
      ...
      <RouterLink v-if="authStore.isStaff" to="/site-settings" class="nav-icon-link" title="Site Settings" aria-label="Site Settings">
    ...
    <main class="feed">
      ...
      >
        <template v-if="item.item_type === 'post'">
          ...
          >
            <img
              v-if="item.data.link_preview?.image_url"
              :src="item.data.link_preview?.image_url"
              class="link-preview-image"
              alt="Link preview"
              loading="lazy"
            />
            <div class="link-preview-content">
              <strong class="link-preview-title">{{ item.data.link_preview?.title }}</strong>
              <p v-if="item.data.link_preview?.description" class="link-preview-description">
                {{ item.data.link_preview?.description }}
              </p>
              <small v-if="item.data.link_preview?.host" class="link-preview-host">{{ item.data.link_preview?.host }}</small>
            </div>
          </div>
        ...
        <template v-else-if="item.item_type === 'suggestion'">
          ...
          <div v-if="isSuggestionExpanded(Number(item.data.user_id || 0))" class="suggestion-post-preview">
            ...
            >
              ...
              >
                <img
                  v-if="post.link_preview?.image_url"
                ...
                />
                <div class="link-preview-content">
                  <strong class="link-preview-title">{{ post.link_preview?.title }}</strong>
                  <p v-if="post.link_preview?.description" class="link-preview-description">
                    {{ post.link_preview?.description }}
                  </p>
                  <small v-if="post.link_preview?.host" class="link-preview-host">{{ post.link_preview?.host }}</small>
                </div>
              </div>
              <div class="post-actions">
